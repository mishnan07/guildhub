...
const RequirementShow = ({ Type, user,show,setInterstedPro,setRequirementId,setHiredPros,profileId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if(profileId){
          setRequirement(response.data.response.filter((item) => item.hired.includes(profileId)));
        }
      } catch (error) {
        console.error(error);
      }
  ...
  const Interested = async(userId,requirementId)=>{
    try {
      console.log(userId);
      const response = await proAxios.post('/intersted',{userId,requirementId})
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4 p-4">
            {requirement.map((item) => (
              ...
              >
                ...
                <div className="mt-4">
                  {Type === "users" ? (
                    <button onClick={()=>details(item.interesteds,item.userId,item._id,item.hired)} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg w-full">
